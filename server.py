...
def changes_page():
    ...
    if not events:
        ...
        if len(snaps) >= 2:
            ...
            computed_live = True
    counts = changes_mod.summary_counts(events)
    return render_template("changes.html", inv=inv, events=events,
                           counts=counts, computed_live=computed_live,
                           snapshot_count=len(diff_mod.list_snapshots()))
...
def api_notify_settings():
    if request.method == "GET":
        return jsonify(notifications.public_settings())
    if DEMO_MODE:
        return jsonify({"ok": False, "error": "デモ環境では通知設定の保存は無効です"}), 403
    data = request.get_json(silent=True) or {}
    to = data.get("to")
    if isinstance(to, str):
        data["to"] = [x.strip() for x in to.replace(";", ",").split(",") if x.strip()]
    try:
        data["weekday"] = int(data.get("weekday", 0))
        data["hour"] = int(data.get("hour", 9))
        data["smtp_port"] = int(data.get("smtp_port", 587))
    except (TypeError, ValueError):
        return jsonify({"ok": False, "error": "weekday/hour/smtp_portは数値で指定してください"}), 400
    notifications.save_settings(data)
    return jsonify({"ok": True, "settings": notifications.public_settings()})
...
def api_notify_preview():
    summary = notifications.build_summary(_load_inventory())
    s = notifications.load_settings()
    html = notifications.render_html(summary, base_url=s.get("base_url") or "")
...
# 週次メール通知スケジューラ（設定が無効なら何もしないアイドルスレッド）
notifications.start_scheduler(_load_inventory)
