...
def _make_prev_inventory():
    """変化ログのデモ用に「1週間前」のインベントリを作る（現状との差分が変化として検出される）。"""
    import copy
    prev = copy.deepcopy(INVENTORY)
    prev["generated_at"] = "2026-06-22T10:00:00+09:00"
    for p in prev["properties"]:
        pid = p["property_id"]
        if pid == "300000006":  # 開発・テスト用: 前回は計測できていた → 計測停止(critical)
            p["is_tracked"] = True
            p["sessions_7d"] = 340
            p["events_7d"] = 2100
        elif pid == "300000002":  # コーポレート: KEが1つ削除された(critical)
            p["key_event_names"] = p["key_event_names"] + ["newsletter_signup"]
            p["key_event_count"] = 4
        elif pid == "300000003":  # ブログ: セッション急減(warn)
            p["sessions_7d"] = 2400
            p["events_7d"] = 19800
    for c in prev["gtm_containers"]:
        cid = c["container_id"]
        if cid == "GTM-DEMO04":   # 新バージョン公開(info)
            c["version_id"] = "6"
        elif cid == "GTM-DEMO10":  # 廃止予定: タグが全て消えた(critical)
            c["tag_count"] = 4
            c["trigger_count"] = 3
    for s in prev["sc_sites"]:
        if s["site_hash"] == "3ce5de2b6aebad5e":  # ブログ: 検索クリック急減(warn)
            s["clicks_28d"] = 520
    return prev
...
def _ensure_demo_data():
    if (_DEMO / "inventory.json").exists():
        return
    # gunicorn 複数ワーカーの同時初期化を防ぐ（mkdir はアトミック）
    try:
        (_DEMO / ".init.lock").mkdir(parents=True, exist_ok=False)
    except FileExistsError:
        import time as _t
        for _ in range(60):  # 他ワーカーの生成完了を最大30秒待つ
            if (_DEMO / "inventory.json").exists():
                return
            _t.sleep(0.5)
        return
    ...
    # スナップショット2件（前週＋今週）→ 変化ログを生成
    import diff as _diff
    import changes as _changes
    snap_dir = _DEMO / "snapshots"
    p_prev = _diff.save_snapshot(_make_prev_inventory())
    p_prev.rename(snap_dir / "20260622T010000Z.json")
